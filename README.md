...
- **文稿配音**：貼上文字，AI 直接合成廣告級配音——八種內建風格
  （磁性男聲／沉穩／活力／知性／溫暖／甜美／戲劇／播報，需要網路），
  加上 **克隆聲線「磁性播音」**（離線 F5-TTS，最自然，需先 `setup_clone.ps1`）
...
# 文稿配音
audio-studio voices                               # 看所有配音風格
audio-studio speak "文稿內容" --voice 磁性男聲 -o 配音.mp3   # 內建 TTS（需網路）
audio-studio speak "文稿內容" --voice 磁性播音 -o 配音.mp3   # 克隆聲線（離線，最自然）
...
- **文稿配音**：聲音是合法的 AI 合成聲，**不是、也不能克隆任何真實
  配音員的嗓音**——克隆真人聲音有人格權法律風險。內建 TTS 走微軟
  神經語音的免費端點，正式大量商用建議申請 Azure 語音服務。
- **克隆聲線「磁性播音」**：參考音是用 F5-TTS 專案內附的自然中文
  語音（隨套件散布、供此用途）降調到男聲音域做的，不是任何特定
  真人。離線運算（第一次下載約 1.3GB 模型），CPU 上一句約 1～3 分鐘。
